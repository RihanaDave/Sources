...
{
    [TestClass]
    public class AutoLayoutAlgorithmTest
    {
        private const LayoutAlgorithmTypeEnum algorithmEnum = LayoutAlgorithmTypeEnum.Auto;
        ...
        public void AvoidNullInputs()
        {
            ...
            AutoLayoutAlgorithm alg = null;

            // Act
            // بررسی عملکرد سازنده (فکتوری) الگوریتم
            try
            {
                alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, null, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, null);
            }
            catch (ArgumentNullException)
            { }
            // Assert
        ...
        public void ComputeWithZeroVertices()
        {
            ...
            AutoLayoutAlgorithm alg = null;
            // Act
            alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, af.CreateLayoutParameters(algorithmEnum));
        ...
        public void ComputeWithRandomContentAndParameters()
        {
            ...
            AutoLayoutParameters parameters = (af.CreateLayoutParameters(algorithmEnum) as AutoLayoutParameters);
            parameters.HorizontalGapBetweenVertices = randomValueGenerator.Next(1000);
            parameters.VerticalGapBetweenVertices = randomValueGenerator.Next(1000);
            alg = (AutoLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, parameters);
            alg.Compute(new System.Threading.CancellationToken());
            // Assert
            Assert.IsNotNull(alg, "عملکرد سازنده الگوریتم (فکتوری) قادر به ایجاد الگوریتم نیست");
            Assert.IsNotNull(alg.VertexPositions, "دیکشنری موقعیت گره‌ها نال است");
