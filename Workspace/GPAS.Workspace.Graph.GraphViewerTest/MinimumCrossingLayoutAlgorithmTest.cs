...
{
    [TestClass]
    public class MinimumCrossingLayoutAlgorithmTest
    {
        private const LayoutAlgorithmTypeEnum algorithmEnum = LayoutAlgorithmTypeEnum.MinimumCrossing;
        ...
        public void AvoidNullInputs()
        {
            ...
            MinimumCrossingLayoutAlgorithm alg = null;

            // Act
            // بررسی عملکرد سازنده (فکتوری) الگوریتم
            try
            {
                alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, null, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, null);
            }
            catch (ArgumentNullException)
            { }
            // Assert
        ...
        public void ComputeWithZeroVertices()
        {
            ...
            MinimumCrossingLayoutAlgorithm alg = null;
            // Act
            alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, af.CreateLayoutParameters(algorithmEnum));
        ...
        public void ComputeWithRandomContentAndParameters()
        {
            ...
            MinimumCrossingLayoutParameters parameters = (af.CreateLayoutParameters(algorithmEnum) as MinimumCrossingLayoutParameters);
            alg = (MinimumCrossingLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, parameters);
            alg.Compute(new System.Threading.CancellationToken());
            // Assert
            Assert.IsNotNull(alg, "عملکرد سازنده الگوریتم (فکتوری) قادر به ایجاد الگوریتم نیست");
            Assert.IsNotNull(alg.VertexPositions, "دیکشنری موقعیت گره‌ها نال است");
