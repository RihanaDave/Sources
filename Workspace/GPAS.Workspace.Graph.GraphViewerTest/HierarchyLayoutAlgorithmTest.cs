...
namespace GPAS.Graph.GraphViewerTest
{
    ...
    {
        private const LayoutAlgorithmTypeEnum algorithmEnum = LayoutAlgorithmTypeEnum.Hierarchy;
        ...
        public void AvoidNullInputs()
        {
            ...
            HierarchyLayoutAlgorithm alg = null;

            // Act
            // بررسی عملکرد سازنده (فکتوری) الگوریتم
            try
            {
                alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, null, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, null);
            }
            catch (ArgumentNullException)
            { }
            // Assert
        ...
        public void ComputeWithZeroVertices()
        {
            ...
            HierarchyLayoutAlgorithm alg = null;
            // Act
            alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, af.CreateLayoutParameters(algorithmEnum));
        ...
        [TestCategory("الگوریتم‌های چینش گراف")]
        [TestMethod]
        public void ComputeWithTwoVerticesWithoutEdge()
        {
            // Assign
            GraphData graph = new GraphData();
            Dictionary<Vertex, Point> verticesPositions = new Dictionary<Vertex, Point>();
            ...
            verticesSizes.Add(v1, new Size(35, 35));
            graph.AddVertex(v1);
            verticesPositions.Add(v2, new Point(75, 75));
            ...
            Assert.AreEqual(verticesPositions[v1].X, alg.VertexPositions[v1].X, "در مواقعی که یالی بین گره‌ها وجود ندارد، طول/عرض تنها گره داده شده به الگوریتم، پس از پردازش آن نباید تغییر کند");
            Assert.AreEqual(verticesPositions[v1].Y, alg.VertexPositions[v1].Y, "در مواقعی که یالی بین گره‌ها وجود ندارد، طول/عرض تنها گره داده شده به الگوریتم، پس از پردازش آن نباید تغییر کند");
            Assert.IsFalse(double.IsNaN(alg.VertexPositions[v1].X), "طول/عرض گره‌های خروجی الگوریتم، می‌بایست عدد مشخصی باشد");
            Assert.IsFalse(double.IsNaN(alg.VertexPositions[v1].Y), "طول/عرض گره‌های خروجی الگوریتم، می‌بایست عدد مشخصی باشد");
            Assert.IsFalse(double.IsInfinity(alg.VertexPositions[v1].X), "طول/عرض گره‌های خروجی الگوریتم، می‌بایست عدد مشخصی باشد");
            Assert.IsFalse(double.IsInfinity(alg.VertexPositions[v1].Y), "طول/عرض گره‌های خروجی الگوریتم، می‌بایست عدد مشخصی باشد");

            Assert.AreEqual(verticesPositions[v2].X, alg.VertexPositions[v2].X, "در مواقعی که یالی بین گره‌ها وجود ندارد، طول/عرض تنها گره داده شده به الگوریتم، پس از پردازش آن نباید تغییر کند");
            Assert.AreEqual(verticesPositions[v2].Y, alg.VertexPositions[v2].Y, "در مواقعی که یالی بین گره‌ها وجود ندارد، طول/عرض تنها گره داده شده به الگوریتم، پس از پردازش آن نباید تغییر کند");
        ...
        public void ComputeWithTwoFullOverlapedVertices()
        {
            ...
            verticesSizes.Add(v2, new Size(35, 35));
            graph.AddVertex(v2);
            graph.AddEdge(new Edge(v1, v2, EdgeDirection.Bidirectional));
            alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, af.CreateLayoutParameters(algorithmEnum));
            alg.Compute(new System.Threading.CancellationToken());
            // Assert
            Assert.IsNotNull(alg, "عملکرد سازنده الگوریتم (فکتوری) قادر به ایجاد الگوریتم نیست");
        ...
        public void ComputeWithRandomContentAndParameters()
        {
            ...
            HierarchyLayoutParameters parameters = (af.CreateLayoutParameters(LayoutAlgorithmTypeEnum.Hierarchy) as HierarchyLayoutParameters);
            alg = (HierarchyLayoutAlgorithm)af.CreateLayoutAlgorithm(LayoutAlgorithmTypeEnum.Hierarchy, graph, verticesPositions, verticesSizes, parameters);
            alg.Compute(new System.Threading.CancellationToken());
            // Assert
            Assert.IsNotNull(alg, "عملکرد سازنده الگوریتم (فکتوری) قادر به ایجاد الگوریتم نیست");
            Assert.IsNotNull(alg.VertexPositions, "دیکشنری موقعیت گره‌ها نال است");
