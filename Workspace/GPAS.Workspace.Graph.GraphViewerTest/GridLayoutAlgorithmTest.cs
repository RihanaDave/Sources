...
namespace GPAS.Graph.GraphViewerTest
{
    ...
    {
        private const LayoutAlgorithmTypeEnum algorithmEnum = LayoutAlgorithmTypeEnum.Grid;
        ...
        public void AvoidNullInputs()
        {
            ...
            GridLayoutAlgorithm alg = null;

            // Act
            // بررسی عملکرد سازنده (فکتوری) الگوریتم
            try
            {
                alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, null, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, null, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, null, null);
            }
            catch (ArgumentNullException)
            { }
            try
            {
                alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, null);
            }
            catch (ArgumentNullException)
            { }
            // Assert
        ...
        public void ComputeWithZeroVertices()
        {
            ...
            GridLayoutAlgorithm alg = null;
            // Act
            alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, af.CreateLayoutParameters(algorithmEnum));
        ...
        public void ComputeWithRandomContentAndParameters()
        {
            ...
            GridLayoutParameters parameters = (af.CreateLayoutParameters(algorithmEnum) as GridLayoutParameters);
            parameters.HorizontalGapBetweenVertices = randomValueGenerator.NextDouble() * 1000;
            parameters.VerticalGapBetweenVertices = randomValueGenerator.NextDouble() * 1000;
            alg = (GridLayoutAlgorithm)af.CreateLayoutAlgorithm(algorithmEnum, graph, verticesPositions, verticesSizes, parameters);
            alg.Compute(new System.Threading.CancellationToken());
            // Assert
            Assert.IsNotNull(alg, "عملکرد سازنده الگوریتم (فکتوری) قادر به ایجاد الگوریتم نیست");
            Assert.IsNotNull(alg.VertexPositions, "دیکشنری موقعیت گره‌ها نال است");
