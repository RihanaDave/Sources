...
namespace ColorHotTrackButton
{
    ...
    public class IconToAvgColorBrushConverter : IValueConverter
    {
        ...
        {
            // Verify if the value is null, if it is return a transparent Fill
            if (value == null)
            {
                ...
                using (Bitmap bitmap = new Bitmap(stream))
                {
                    ...
                    for (int x = 0; x < bitmap.Width; x++)
                    {
                        for (int y = 0; y < bitmap.Height; y++)
                        {
                            ...
                            // verify if the color is transparent because a color of #00000000 would darker the Fill
                            if (pixel.A > 0x00)
                                colors.Add(pixel);
                        }
                    }
            ...
            {
                // If any error occours return a Tranparent Fill
                return System.Windows.Media.Brushes.Transparent;
            }
        }
