...
namespace GPAS.FeatureTest.PropertyRetrievalTests
{
    public class KWObjectWithPropertiesForTest
    {
        ...
        public static async Task<KWObjectWithPropertiesForTest> Create(string objTypeURI, string label, Dictionary<string, List<string>> properties,
                                                                                List<GeoTimeEntityRawData> geoTimeProperties)
        {
            ...
            {
                KWProperty kWProperty = PropertyManager.CreateNewPropertyForObject(kWObject,
                    OntologyProvider.GetOntology().GetDateRangeAndLocationPropertyTypeUri(), GeoTime.GetGeoTimeStringValue(gtprp));
                outProp.Add(kWProperty);
            }
