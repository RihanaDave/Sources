...
namespace GPAS.SearchServer.ServerManager
{
    class Program
    {
        ...
        {
            Console.WriteLine("Search Server Manager console");
            Console.WriteLine();
            Console.WriteLine("-R | --Reset [-ConcurrencyLevel=n] [-BatchSize=n] [-StartObjectID=n] [-LastObjectID=n] [-StartDataSourceID=n] [-LastDataSourceID=n]");
            Console.WriteLine();
            Console.WriteLine("Provides search server data reset (reindex entire or part of objects' data)");
            Console.WriteLine();
            Console.WriteLine("\t-ConcurrencyLevel");
            Console.WriteLine("\t\tIndicates concorrency level of retrieve and sync.");
            Console.WriteLine("\t\tDefault: {read from Search Service configuration}");
            Console.WriteLine();
            Console.WriteLine("\t-BatchSize");
            Console.WriteLine("\t\tObjects retrieve and sync. Batch size");
            Console.WriteLine("\t\tDefault: {read from Search Service configuration}");
            Console.WriteLine();
            Console.WriteLine("\t-StartObjectID");
            Console.WriteLine("\t\tStart Objects' sync. from a specified Object (ID)");
            Console.WriteLine("\t\tDefault: 1");
            Console.WriteLine();
            Console.WriteLine("\t-LastObjectID");
            Console.WriteLine("\t\tEnds the Objects' sync. process to the specified Object (ID)");
            Console.WriteLine("\t\tDefault: {Last stored Object ID}");
            Console.WriteLine();
            Console.WriteLine("\t-StartDataSourceID");
            Console.WriteLine("\t\tStart Data-Sources' sync. from a specified Data-Source (ID)");
            Console.WriteLine("\t\tDefault: 1");
            Console.WriteLine();
            Console.WriteLine("\t-LastDataSourceID");
            Console.WriteLine("\t\tEnds the Data-Sources' sync. process to the specified Data-Source (ID)");
            Console.WriteLine("\t\tDefault: {Last stored Data-Source ID}");
            Console.WriteLine();
            Console.WriteLine("-RO | --ResetOntology");
            Console.WriteLine();
            Console.WriteLine("Provides search server ontology synchronization");
            Console.WriteLine();
            Console.WriteLine("-S | --Synchronize");
            Console.WriteLine();
            Console.WriteLine("Try synchronize all search server unsync. indexes");
            Console.WriteLine();
        ...
        private static void DoOperation(ConsoleInput input)
        {
            try
            {
                if (input is ResetArguments)
                {
                    ...
                    if (resetArguments.IsFirstObjectIDSet)
                    {
                        indexProvider.FirstObjectID = resetArguments.FirstObjectID;
                    }
                    if (resetArguments.IsFirstDataSourceIDSet)
                    {
                        indexProvider.FirstDataSourceID = resetArguments.FirstDataSourceID;
                    }
                    if (resetArguments.IsLastObjectIDSet)
                    {
                        indexProvider.LastObjectID = resetArguments.LastObjectID;
                    }
                    if (resetArguments.IsLastDataSourceIDSet)
                    {
                        indexProvider.LastDataSourceID = resetArguments.LastDataSourceID;
                    }
                ...
                else if (input is SynchronizeOntologyArguments)
                {
                    Console.WriteLine("Synchronize ontology last changes...");
                    SearchEngineProvider.Init();
                    Console.WriteLine("Ontology synchronized.");
                }
                else
                {
        ...
        private static ConsoleInput CheckArguments(string[] args)
        {
            ...
            else if (args[0].Equals("-RO") || args[0].Equals("--ResetOntology"))
            {
                SynchronizeOntologyArguments result = new SynchronizeOntologyArguments();
                result.SynchronizeOntologyLastChanges = true;
                return result;
            }
            else if (args[0].Equals("-R") || args[0].Equals("--Reset"))
            {
                ...
                for (int i = 1; i < args.Length; i++)
                {
                    ...
                    else if (arg.ToLower().StartsWith("-startobjectid="))
                    {
                        result.IsFirstObjectIDSet = true;
                        result.FirstObjectID = int.Parse(arg.Substring(arg.IndexOf('=') + 1));
                    }
                    else if (arg.ToLower().StartsWith("-lastobjectid="))
                    {
                        result.IsLastObjectIDSet = true;
                        result.LastObjectID = int.Parse(arg.Substring(arg.IndexOf('=') + 1));
                    }
                    else if (arg.ToLower().StartsWith("-startdatasourceid="))
                    {
                        result.IsFirstDataSourceIDSet = true;
                        result.FirstDataSourceID = int.Parse(arg.Substring(arg.IndexOf('=') + 1));
                    }
                    else if (arg.ToLower().StartsWith("-lastdatasourceid="))
                    {
                        result.IsLastDataSourceIDSet = true;
                        result.LastDataSourceID = int.Parse(arg.Substring(arg.IndexOf('=') + 1));
                    }
                    else
                    {
        ...
        private static void LoadConfigFileFromWebService()
        {
            ...
            Configuration serviceConfig = WebConfigurationManager.OpenWebConfiguration("\\", "Search Service");
