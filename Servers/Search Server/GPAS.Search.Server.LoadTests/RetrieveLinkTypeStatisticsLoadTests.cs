...
namespace GPAS.Search.Server.LoadTests
{
    ...
    public class RetrieveLinkTypeStatisticsLoadTests : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.SearchServer;
        public TestClassType TestClassType { get; set; } = TestClassType.RetrieveLinkTypeStatistics;
        public string Title { get; set; } = "RetrieveLinkTypeStatistics";
        ...
        [TestMethod]
        public void RetrieveLinkTypeStatistics()
        {
            try
            {
                #region Assign
                ...
                LinkTypeStatistics linkTypeStatistics = null;
                ...
                using (ShimsContext.Create())
                {
                    ...
                    for (int i = 0; i < QueryCount; i++)
                    {
                        ...
                        Watch.Start();
                        linkTypeStatistics = serviceClient.RetrieveLinkTypeStatistics(queryByteArray, authorizationParametters);
                        Watch.Stop();
                ...
                WriteLog("RetrieveLinkTypeStatistics " + QueryCount + " Batch with different query.\n" +
        ...
        public async Task<List<LoadTestResult>> RunTests(long startStore, long endStore, long startRetrieve, long endRetrieve, CancellationToken tokenSource)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Retrieve LinkType Statistics",
                    Description = "Retrieve LinkType Statistics with different query.",
            ...
            for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
            {
                ...
                for (int i = 0; i < testResults.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            case 0:
                                await Task.Run(() => RetrieveLinkTypeStatistics());
                                break;
                            default:
                                break;
                        }
