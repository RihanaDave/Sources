...
namespace GPAS.Search.Server.LoadTests
{
    ...
    public class RetrievePropertyValueStatisticsLoadTests : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.SearchServer;
        public TestClassType TestClassType { get; set; } = TestClassType.RetrievePropertyValueStatistics;
        public string Title { get; set; } = "RetrievePropertyValueStatistics";
        ...
        [TestMethod]
        public void RetrievePropertyValueStatistics()
        {
            long totalRetrivedPropertyValuesFrequency = 0;
            ...
            try
            {
                ...
                for (int i = 0; i < QueryCount; i++)
                {
                    ...
                    {
                        FormulaStep formulaStep = GenerateTypeBasedDrillDown(true);
                ...
                using (ShimsContext.Create())
                {
                    ...
                    for (int i = 0; i < QueryCount; i++)
                    {
                        ...
                        string pName = properties[Random.Next(0, properties.Count - 1)];

                        //query.FormulaSequence.Add(GeneratePropertyValueBasedDrillDown(pName, 1));
                        ...
                        Watch.Start();
                        PropertyValueStatistics result = serviceClient.RetrievePropertyValueStatistics(queryByteArray, pName, startOffset, (int)BatchItems, minimumCount, authorizationParametters);
                        Watch.Stop();
                        totalTime += Watch.Elapsed;
                        Watch.Reset();

                        if (result != null)
                            totalRetrivedPropertyValuesFrequency += result.TotalResultsCount;
                    }
                }
                #endregion
                ...
                WriteLog("Retrieved PropertyValue Frequency Statistics : " + totalRetrivedPropertyValuesFrequency + "\n" +
        ...
        public async Task<List<LoadTestResult>> RunTests(long startStore, long endStore, long startRetrieve, long endRetrieve, CancellationToken tokenSource)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Retrieve PropertyValue Statistics",
                    Description = "Retrieve PropertyValue Statistics with different query for all properties.",
            ...
            for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
            {
                ...
                for (int i = 0; i < testResults.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            case 0:
                                await Task.Run(() => RetrievePropertyValueStatistics());
                                break;
                            default:
                                break;
                        }
