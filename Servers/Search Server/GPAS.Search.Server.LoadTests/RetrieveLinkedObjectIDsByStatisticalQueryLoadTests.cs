...
namespace GPAS.Search.Server.LoadTests
{
    ...
    public class RetrieveLinkedObjectIDsByStatisticalQueryLoadTests : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.SearchServer;
        public TestClassType TestClassType { get; set; } = TestClassType.RetrieveLinkedObjectIDsByStatistical;
        public string Title { get; set; } = "RetrieveLinkedObjectIDsByStatistical";
        ...
        [TestMethod]
        public void RetrieveLinkedObjectIDsByStatisticalQuery()
        {
            long retrivedLinkedObjectsCount = 0;
            ...
            try
            {
                ...
                for (int i = 0; i < QueryCount; i++)
                {
                    ...
                    {
                        FormulaStep formulaStep = GenerateLinkBasedDrillDown();
                ...
                using (ShimsContext.Create())
                {
                    ...
                    for (int i = 0; i < QueryCount; i++)
                    {
                        ...
                        Watch.Start();
                        var result = serviceClient.RetrieveLinkedObjectIDsByStatisticalQuery(queryByteArray, (int)BatchItems, authorizationParametters);
                        Watch.Stop();
                        totalTime += Watch.Elapsed;
                        Watch.Reset();

                        if (result != null)
                            retrivedLinkedObjectsCount += result.Length;
                    }
                }
                #endregion
                ...
                WriteLog("Retrieved " + retrivedLinkedObjectsCount + " LinkObjects in " + QueryCount + " requests with different query.\n" +
        ...
        public async Task<List<LoadTestResult>> RunTests(long startStore, long endStore, long startRetrieve, long endRetrieve, CancellationToken tokenSource)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Retrieve Linked Object IDs By Statistical Query",
                    Description = "Retrieve Linked Object IDs By Statistical Query with different query.",
            ...
            for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
            {
                ...
                for (int i = 0; i < testResults.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            case 0:
                                await Task.Run(() => RetrieveLinkedObjectIDsByStatisticalQuery());
                                break;
                            default:
                                break;
                        }
