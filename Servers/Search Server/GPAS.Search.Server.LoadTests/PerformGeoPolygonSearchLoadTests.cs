...
namespace GPAS.Search.Server.LoadTests
{
    ...
    public class PerformGeoPolygonSearchLoadTests : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.SearchServer;
        public TestClassType TestClassType { get; set; } = TestClassType.PerformGeoPolygonSearch;
        public string Title { get; set; } = "PerformGeoPolygonSearch";
        ...
        [TestMethod]
        public void PerformGeoPolygonSearch()
        {
            ...
            try
            {
                ...
                using (ShimsContext.Create())
                {
                    ...
                    for (int i = 0; i < QueryCount; i++)
                    {
                        ...
                        Watch.Start();
                        List<long> result = serviceClient.PerformGeoPolygonSearch(polygonSearchCriteriaList[i], (int)BatchItems, authorizationParametters).Select(a => a.Id).ToList();
                        Watch.Stop();
        ...
        public async Task<List<LoadTestResult>> RunTests(long startStore, long endStore, long startRetrieve, long endRetrieve, CancellationToken tokenSource)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Perform Geo Polygon Search",
                    Description = "Perform Geo Polygon Search with different polygon shapes.",
            ...
            for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
            {
                ...
                for (int i = 0; i < testResults.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            case 0:
                                await Task.Run(() => PerformGeoPolygonSearch());
                                break;
                            default:
                                break;
                        }
