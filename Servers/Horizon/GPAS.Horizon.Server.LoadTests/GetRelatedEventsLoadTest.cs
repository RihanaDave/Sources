...
namespace GPAS.Horizon.Server.LoadTests
{
    ...
    public class GetRelatedEventsLoadTest : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.HorizonServer;
        public TestClassType TestClassType { get; set; } = TestClassType.EventRetrival;
        public string Title { get; set; } = "Retrieve events ";
        ...
        {
            //GetRelatedEventsLoadTest getRelatedEventsLoadTest = new GetRelatedEventsLoadTest();
            //getRelatedEventsLoadTest.Publish();          
        }
        ...
        {
            GetRelatedEventsLoadTest getRelatedEventsLoadTest = new GetRelatedEventsLoadTest();
            getRelatedEventsLoadTest.ClearAllData();
        }
        ...
        public bool FindRelatedEvents()
        {
            ...
            using (ShimsContext.Create())
            {
                   ...
                   Random.Next()).Take(RetrieveItemsCount).Select(Convert.ToInt64).ToArray();
                    ...
                    try
                    {
                        ...
                        List<RelationshipBasedResultsPerSearchedObjects> result =
                            HorizonService.FindRelatedEvents(listOfObjects, resultLimit, authorization);
                        timer.Stop();

                        WriteLog($"{result.Count} related event(s) for batch = {batch} and BatchCount = {BatchCount} was retrieved.\n");

                        OnTestCaseProgressChanged(new TestCaseProgressChangedEventArgs(BatchCount.ToString(), batch.ToString()));
                        #region Assert

                        retrievalSucceeded = RelatedEventsDataValidation(result);
                ...
                if (retrievalSucceeded)
                {
                    ...
                    WriteLog(" ************************************************************************************************************ ");
                    WriteLog("Retrieve events " + BatchCount + " times in average " +
        ...
        private bool RelatedEventsDataValidation(List<RelationshipBasedResultsPerSearchedObjects> retrievedData)
        {
            ...
            foreach (var currentRelation in retrievedData)
            {
                ...
                if (remainingOfSearchedObjIdDevidedByBatchItem == 0)
                {
                    ...
                    {
                        validation = false;
                        string failedMassage = string.Format("Related events retrieval failed for id = {0}", currentRelation.SearchedObjectID);
...
#pragma warning restore CS0168 // The variable 'e' is declared but never used
            ...
            CancellationToken token)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Retrieve related events",
                    Description = "Retrieve related events",
                ...
                for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
                {
                    ...
                    for (int i = 0; i < testsMethodCount; i++)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                case 0:
                                    await Task.Run(() => FindRelatedEvents());
                                    break;
                            }
