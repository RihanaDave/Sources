...
namespace GPAS.Horizon.Server.LoadTests
{
    ...
    public class GetRelatedDocumentsLoadTest : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.HorizonServer;
        public TestClassType TestClassType { get; set; } = TestClassType.DocumentRetrival;
        public string Title { get; set; } = "Retrieve documents ";

#pragma warning disable CS0414 // The field 'GetRelatedDocumentsLoadTest.maxObjectsCount' is assigned but its value is never used
        private int maxObjectsCount = 100;
#pragma warning restore CS0414 // The field 'GetRelatedDocumentsLoadTest.maxObjectsCount' is assigned but its value is never used
#pragma warning disable CS0414 // The field 'GetRelatedDocumentsLoadTest.rangesCount' is assigned but its value is never used
        private int rangesCount = 1;
#pragma warning restore CS0414 // The field 'GetRelatedDocumentsLoadTest.rangesCount' is assigned but its value is never used
#pragma warning disable CS0414 // The field 'GetRelatedDocumentsLoadTest.items' is assigned but its value is never used
        private long items = 1000;
#pragma warning restore CS0414 // The field 'GetRelatedDocumentsLoadTest.items' is assigned but its value is never used
        private Stopwatch timer;

        [ClassInitialize]
        public static void PublishObjectsWithOnePropertyBeforeTests(TestContext testContext)
        {
            //GetRelatedDocumentsLoadTest getRelatedDocumentsLoadTest = new GetRelatedDocumentsLoadTest();            
            //getRelatedDocumentsLoadTest.Publish();
        }
        ...
        {
            //GetRelatedDocumentsLoadTest getRelatedDocumentsLoadTest = new GetRelatedDocumentsLoadTest();
            //getRelatedDocumentsLoadTest.ClearAllData();
        }
        ...
        private void Publish()
        {
            ...
            WriteLog(" ******************  Publish documents load tests  ****************** ");

            PublishLoadTests publishLoadTests = new PublishLoadTests();
            publishLoadTests.TestCaseProgressChanged += PublishLoadTests_TestCaseProgressChanged;
            publishLoadTests.PublishRelatedDocumentsWithSingleConnections();
        }
        ...
        [TestMethod]
        public bool FindRelatedDocuments()
        {
            #region Assign 
            ...
            using (ShimsContext.Create())
            {
                ...
                for (int batch = 0; batch < BatchCount; batch++)
                {
                    ...
                    try
                    {
                        ...
                        List<RelationshipBasedResultsPerSearchedObjects> result =
                            HorizonService.FindRelatedDocuments(listOfObjects, resultLimit, authorization);
                        timer.Stop();

                        WriteLog($"{result.Count} related document(s) appeared in events for batch = {batch} and BatchCount = {BatchCount} was retrieved.\n");

                        OnTestCaseProgressChanged(new TestCaseProgressChangedEventArgs(BatchCount.ToString(), batch.ToString()));

                        #region Assert
                        retrievalSucceeded = RelatedDocumentsDataValidation(result);
                    ...
                    {
                        string logMessage = string.Format(ex.Message + "(BatchCount = {0}/{1}, BatchItems = {2})", batch.ToString(), BatchCount.ToString(), BatchItems);
                        WriteLog(logMessage);
                        break;
                    }
                }
                ...
                if (retrievalSucceeded)
                {
                    ...
                    WriteLog(" ************************************************************************************************************ ");
                    WriteLog("Retrieve single connected documents " + BatchCount + " times in average " +
        ...
        private bool RelatedDocumentsDataValidation(List<RelationshipBasedResultsPerSearchedObjects> retrievedData)
        {
            bool validation = true;

            foreach (var currentRelation in retrievedData)
            {
                long remainingOfSearchedObjIdDevidedByBatchItem = (currentRelation.SearchedObjectID % 2);

                if (remainingOfSearchedObjIdDevidedByBatchItem == 1)
                {
                    if (currentRelation.NotLoadedResults.Count() != 1)
                    {
                        validation = false;
                        string failedMassage = string.Format("Related documents retrieval failed for id = {0}", currentRelation.SearchedObjectID);
                        WriteLog(failedMassage);
                    }
                }
                else
                {
                    if (currentRelation.NotLoadedResults.Count() != 0)
                    {
                        validation = false;
                        string failedMassage = string.Format("Related documents retrieval failed for id = {0}", currentRelation.SearchedObjectID);
                        WriteLog(failedMassage);
            ...
            CancellationToken token)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    ...
                    Description = "Retrieve single connected documents",
            ...
            for (long storeIndex = startStore; storeIndex <= endStore; storeIndex *= 10)
            {
                ...
                OnStepsProgressChanged(new StepsProgressChangedEventArgs(totalStepsCount, currentStepNumber.ToString(),
                    "Publish single connected documents", testResults));
                ...
                for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
                {
                    ...
                    for (int i = 0; i < testsMethodCount; i++)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                case 0:
                                    await Task.Run(() => FindRelatedDocuments());
                                    break;
                            }
