...
namespace GPAS.Horizon.Server.LoadTests
{
    ...
    public class GetRelatedEntitiesAppearedInEventsLoadTest : BaseLoadTest, ILoadTestCategory
    {
        public ServerType ServerType { get; set; } = ServerType.HorizonServer;
        public TestClassType TestClassType { get; set; } = TestClassType.IntermediateEventRetrival;
        public string Title { get; set; } = "Retrieve entities appeared in events ";

#pragma warning disable CS0414 // The field 'GetRelatedEntitiesAppearedInEventsLoadTest.maxObjectsCount' is assigned but its value is never used
        private int maxObjectsCount = 100;        
#pragma warning restore CS0414 // The field 'GetRelatedEntitiesAppearedInEventsLoadTest.maxObjectsCount' is assigned but its value is never used
        private Stopwatch timer;

        [ClassInitialize]
        public static void PublishObjectsWithOnePropertyBeforeTests(TestContext testContext)
        {
            //GetRelatedEntitiesAppearedInEventsLoadTest getRelatedEntitiesAppearedInEventsLoadTest = new GetRelatedEntitiesAppearedInEventsLoadTest();
            //getRelatedEntitiesAppearedInEventsLoadTest.Publish(); 
        }
        ...
        {
            //GetRelatedEntitiesAppearedInEventsLoadTest getRelatedEntitiesAppearedInEventsLoadTest = new GetRelatedEntitiesAppearedInEventsLoadTest();
            //getRelatedEntitiesAppearedInEventsLoadTest.ClearAllData();
        }
        ...
        [TestMethod]
        public bool FindRelatedEntitiesAppearedInEvents()
        {
            #region Assign 
            ...
            using (ShimsContext.Create())
            {
                ...
                {
                    return new List<string>() { "مکالمه_تلفنی" };
                };
                ...
                for (int batch = 0; batch < BatchCount; batch++)
                {
                    ...
                    {
                        { "مکالمه_تلفنی", listOfObjectsIds }
                    };

                    try
                    {
                        timer.Start();
                        List<EventBasedResultsPerSearchedObjects> result = 
                            HorizonService.FindRelatedEntitiesAppearedInEvents(listOfObjects, resultLimit, authorization);
                        timer.Stop();
                        ...
                        retrievalSucceeded = RelatedEntitiesAppearedInEventsDataValidation(result);
                    ...
                    {
                        string logMessage = string.Format(ex.Message + "(BatchCount = {0}/{1}, BatchItems = {2})", batch.ToString(), BatchCount, BatchItems);
                        WriteLog(logMessage);
                        break;
                    }
                }
                ...
                if (retrievalSucceeded)
                {
                    ...
                    WriteLog(" ************************************************************************************************************ ");
                    WriteLog("Retrieve related entities appeared in events " + BatchCount + " times in average " +
        ...
        private bool RelatedEntitiesAppearedInEventsDataValidation(List<EventBasedResultsPerSearchedObjects> retrievedData)
        {
            ...
            foreach (var currentRelation in retrievedData)
            {
                ...
                if (remainingOfSearchedObjIdDevidedByBatchItem == 0)
                {
                    ...
                    {
                        validation = false;
                        string failedMassage = string.Format("Related entities appeared in events retrieval failed for id = {0}", currentRelation.SearchedObjectID);
                ...
                else
                {
                    ...
                    {
                        if (Math.Abs(currentRelation.NotLoadedResults[0].TargetObjectID - currentRelation.SearchedObjectID) != 2)
                        {
                            validation = false;
                            string failedMassage = string.Format("Related entities appeared in events retrieval failed for id = {0}", currentRelation.SearchedObjectID);
                            WriteLog(failedMassage);
...
#pragma warning restore CS0168 // The variable 'e' is declared but never used
            ...
            CancellationToken token)
        {
            ...
            var testResults = new List<LoadTestResult>
            {
                new LoadTestResult
                {
                    Title = "Retrieve related entities appeard in events",
                    Description = "Retrieve related entities appeard in events",
            ...
            for (long storeIndex = startStore; storeIndex <= endStore; storeIndex *= 10)
            {
                ...
                OnStepsProgressChanged(new StepsProgressChangedEventArgs(totalStepsCount, currentStepNumber.ToString(),
                    "Publish entities appeared in events", testResults));
                ...
                for (long retrieveIndex = startRetrieve; retrieveIndex <= endRetrieve; retrieveIndex *= 10)
                {
                    ...
                    for (int i = 0; i < testsMethodCount; i++)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                case 0:
                                    await Task.Run(() => FindRelatedEntitiesAppearedInEvents());
                                    break;
                            }
